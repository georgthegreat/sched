...
class MpiCommand(LocalCommand):

	def __init__(self, command, inputs, outputs, **attrib):
		super().__init__(command, inputs, outputs)

		self.min_nodes = int(attrib["min-nodes"])
		self.max_nodes = int(attrib["max-nodes"])

	@property
	def type(self):
		return CommandType.Mpi
...
class MpiFileDivisibleCommand(FileDivisibleCommand):

	def __init__(self, command, inputs, outputs, **attrib):
		super().__init__(command, inputs, outputs)

		self.mincpus = int(attrib["mincpus"])
		self.maxcpus = int(attrib["maxcpus"])
...
class CommandType(object):
	...
	_dict = {
		...
		"mpi": Mpi,
		"mpi-file-divisible": MpiFileDivisible
	}
	...
	type_to_class = {
		...
		Mpi: MpiCommand,
		MpiFileDivisible: MpiFileDivisibleCommand
	}
